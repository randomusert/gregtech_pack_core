# version 0.1
## WARNING!
This mod is in beta and in early development.

## features
* custom machinery machine called Alloyer
* item called Minecraftium
* Incomplete translation for fi-fi
* Recipe for Minecraftium with Alloyer
* Recipe for Alloyer

## Dependencies
### required
* Custom machinery
* Mekanism
* GTCEu modern

### optional
* Just Enough Items (JEI)

# version 0.2
## warning!
This mod is in beta and in early development

## What's new
* added a machine called Ium crafter
* more crafting recipes, more is coming
* added item called gregium

# version 0.3
## warning!
This mod is in beta and in early development
## What's new
* added incomplete GTCEu fi_fi translation
* Patchouli book for en_us and fi_fi(fi_fi entries and categories coming soon)
* added a creative tier material called Singularity Alloy
* added a machine called ultimate crafter or internally ultcrafter
* added a block called compressed iron block (internally) / compressed block of iron


## bug fixes
* fixed 1 recipe.
* fixed a issue where the singularity alloy full stack would kickthe player from the singleplayer world. not on public versions

# version 0.4
## warning!
This mod is in beta and in early development. Expect bugs, crashes and other issues

## What's new
* Refined Storage Disk and part for storing items
* Recipes for the said Disks.
* Item tag datagen stuff behind the scenes.
* Datagen behind the scenes.
* Build and Run python scripts
## notes about this update
This update does a lot of Backend stuff. This helps to develop this mod.

# version 0.5
## warning!
This mod is in beta and in early development. Expect bugs, crashes and other issues.

## Added
* added a new material
* added materials creative tab
* added a ore
* World gen and a JEI plugin
* advancements
* continued on patchouli guide book
* Greenhouse multiblock machine from modular machinery reborn

## fixed
* data generation recipe provider registration
* recipe for singularity alloy
* missing translations for xenoverd ingot, scrap and ore

## changed
* a brand new texture for minecraftium


# version 0.6